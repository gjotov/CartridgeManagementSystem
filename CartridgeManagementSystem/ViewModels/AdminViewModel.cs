        ...
        private void AddUser(object parameter)
        {
            ...
            if (addUserWindow.ShowDialog() == true)
            {
                using (SQLiteConnection conn = new SQLiteConnection("Data Source=cartridge_management.db"))
                {
                    ...
                    using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
                    {
                        cmd.Parameters.AddWithValue("@username", addUserWindow.Username);
                        cmd.Parameters.AddWithValue("@password", addUserWindow.Password);
                        cmd.Parameters.AddWithValue("@role", addUserWindow.Role);
                        cmd.ExecuteNonQuery();
                    }
                }
                LoadUsers();
